...
class Customer extends CI_Controller {
	...
	public function index()
	{
		...
		if($customer_data['customer_id']!=NULL && $this->session->userdata('role')!=3){
	...
	public function add_customer()
	{
		...
		if($customer_data['reference']!="NULL"){
		...
		$customer_data['additional_charge']						=	$this->input->post('additional_charge','',TRUE);
	...
	public function customer_detail(){
		...
		$zone_detail 							=	$this->zone_model->get_zone_by_id($customer_data['customer_detail']->zone_id);

		$customer_data['head_of_sales']			=	$this->employee_model->get_employee_by_id($zone_detail->head_of_sales_id)->employee_name;
